...
for turn,file in enumerate(file_sets):
    ...
    db.to_csv(path_or_buf = '../time_series_one/'+file+'_load'+'.csv', columns = ['LoadScore',], index=False, index_label=False, header = False)
    db.to_csv(path_or_buf = '../time_series_one/'+file+'_perf'+'.csv', columns = ['PerfScore',], index=False, index_label=False, header = False)
