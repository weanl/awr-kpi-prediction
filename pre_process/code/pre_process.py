...
def regression(training_data, scores):
    ...
    X_train, X_test, y_train, y_test = train_test_split(training_data, scores_load, test_size=0.2)
...
if  __name__ == '__main__':
    ...
    for turn,file in enumerate(sets):
        ...
        feature_select_load["lasso"][turn] = feature_selection("lasso",instance_data,scores_load)
        ...
        instance_data = db[feature]
        # save select features
        feature_select_load["pearson"][turn] = feature_selection("pearson",instance_data,scores_load)
        ...
        instance_data = db[feature]
        # save select features
        feature_select_load["reliefF"][turn] = feature_selection("reliefF",instance_data,np.array(levels_load,np.int32))
        ...
        instance_data = db[feature]
        # save select features
        feature_select_perf["lasso"][turn]= feature_selection("lasso",instance_data,scores_perf)
        ...
        instance_data = db[feature]
        # save select features
        feature_select_perf["pearson"][turn]= feature_selection("pearson",instance_data,scores_perf)
        ...
        instance_data = db[feature]
        # save select features
        feature_select_perf["reliefF"][turn] = feature_selection("reliefF", instance_data, np.array(levels_perf, np.int32))
    ...
    plot_hist(scores_perf_aggre, (7, 5), "performance score", "# of performance score", 20, "histograph_of_perf_scores",0.65,"percentage")
    plot_hist(levels_perf_aggre, (5, 5), "performance level", "# of performance level",4, "histograph_of_perf_levels",0.7,"percentage")
